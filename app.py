...
# ---------------------------------------------------------------------------
    ...
    st.caption("Scan beberapa saham sekaligus, dikelompokkan per sektor IDX-IC, "
               "supaya kandidat bullish di tiap sektor tidak 'tertutup' oleh sektor yang sedang rally.")

    st.info("📂 Klasifikasi sektor mengikuti **IDX-IC** (11 sektor resmi BEI sejak 2021). "
            "Daftar saham di sini adalah kurasi manual saham likuid per sektor, BUKAN seluruh "
            "universe BEI (~900+ saham) — silakan edit daftar di bawah jika ingin menambah saham lain.")

    mode = st.radio(
        "Mode tampilan hasil",
        ["🗂️ Top Bullish per Sektor", "🏆 Ranking Global (semua digabung)"],
        horizontal=True,
    )

    sector_choices = st.multiselect(
        "Pilih sektor yang ingin di-scan",
        options=sec.sector_options(),
        default=sec.sector_options(),
        help="Defaultnya semua 11 sektor IDX-IC dipilih. Kurangi pilihan untuk screening lebih cepat.",
    )

    selected_codes = [code for code, info in sec.SECTORS.items()
                       if f"{info['emoji']} {info['name']}" in sector_choices]
    sector_ticker_map = sec.tickers_by_sector(selected_codes, with_suffix=False)
    default_tickers_flat = [t for tickers in sector_ticker_map.values() for t in tickers]
    default_list = ", ".join(default_tickers_flat)

    tickers_text = st.text_area(
        "Daftar kode saham yang akan di-scan (pisahkan dengan koma — bisa ditambah/dihapus manual)",
        default_list, height=100,
    )
    ...
    with col3:
        top_n_sector = st.number_input("Top-N per sektor", 1, 10, 3,
                                         help="Hanya dipakai pada mode 'Top Bullish per Sektor'.")

    est_minutes = round(len(tickers) * (3 if use_ml_scr else 0.5) / 60, 1)
    st.warning(f"Akan menganalisis {len(tickers)} saham dari {len(sector_ticker_map)} sektor. "
               f"Estimasi waktu proses: ~{est_minutes} menit jika ML diaktifkan (training per saham).")

    if st.button("🚀 Jalankan Screening", type="primary"):
        if not tickers:
            st.error("Daftar saham kosong. Pilih minimal satu sektor atau ketik kode saham manual.")
        else:
            progress = st.progress(0, text="Memulai screening...")
            with st.spinner("Mengambil & menganalisis data semua saham..."):
                result_df = scr.screen_bullish_candidates(tickers, horizon=int(horizon_scr), use_ml=use_ml_scr)
            progress.progress(100, text="Selesai")

            if result_df.empty:
                st.error("Tidak ada hasil. Cek kode saham yang dimasukkan atau coba lagi (kemungkinan rate-limit Yahoo Finance).")
            else:
                n_failed = len(tickers) - len(result_df)
                st.success(f"Berhasil menganalisis {len(result_df)} dari {len(tickers)} saham yang diminta"
                           + (f" ({n_failed} gagal/data tidak cukup)." if n_failed > 0 else "."))

                # --- Ringkasan suhu per sektor ---
                summary_df = scr.sector_summary(result_df)
                if not summary_df.empty:
                    st.subheader("🌡️ Suhu Tiap Sektor (skor rata-rata)")
                    fig_sector = go.Figure(go.Bar(
                        x=summary_df["avg_score"], y=summary_df["sector"],
                        orientation="h",
                        marker_color=["#1a9850" if v > 0 else "#d73027" for v in summary_df["avg_score"]],
                        text=summary_df["avg_score"], textposition="outside",
                    ))
                    fig_sector.update_layout(height=max(250, 40 * len(summary_df)),
                                              xaxis_title="Skor rata-rata (-100 s.d 100)",
                ...
                if mode == "🗂️ Top Bullish per Sektor":
                    st.subheader(f"🗂️ Top {int(top_n_sector)} Saham Paling Bullish — per Sektor")
                    grouped = scr.top_n_per_sector(result_df, top_n=int(top_n_sector))
                    for sector_name in grouped["sector"].unique():
                        sub = grouped[grouped["sector"] == sector_name].reset_index(drop=True)
                        best = sub.iloc[0]
                        with st.expander(f"{sector_name} — terbaik: {best['ticker']} ({best['decision']}, skor {best['score']})",
                                          expanded=True):
                            st.dataframe(
                                sub.drop(columns=["sector"]),
                                use_container_width=True, hide_index=True,
                                column_config={
                                    "score": st.column_config.ProgressColumn(
                                        "Skor Sinyal", min_value=-100, max_value=100, format="%.1f"
                                    )
                                }
                            )
                else:
                    st.subheader("🏆 Ranking Global (semua sektor digabung)")
                    st.dataframe(
                        result_df,
                        use_container_width=True, hide_index=True,
                        column_config={
                    ...
                    st.subheader("🏆 Top 3 Kandidat Paling Bullish (keseluruhan)")
                    for _, row in top3.iterrows():
                        st.markdown(f"**{row['ticker']}** ({row['sector']}) — {row['decision']} (skor {row['score']}) — "
                                    f"Harga: Rp {row['last_price']:,.0f}, Proyeksi: Rp {row['projected_price']:,.0f} "
                                    f"(range Rp {row['range_lower']:,.0f} - Rp {row['range_upper']:,.0f})")

                st.caption("Sinyal ini alat bantu screening, bukan rekomendasi final. Selalu cek fundamental, "
                           "ARA/ARB, dan berita terbaru sebelum transaksi.")
